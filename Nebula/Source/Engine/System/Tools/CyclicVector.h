...
class CyclicVector
{
	private:
		...
		/**
		Resolves the active index. m_iNumElements must be correct prior to calling this function. */
		void resolveActiveIndex()
		{
			if (m_iNumElements == 0)
			{
				m_iActiveIndex = 0;
			}
			else if (m_iActiveIndex >= m_iNumElements)
			{
		...
		public:
			...
			/**
			Constructs a cyclic vector of size 5. */
			CyclicVector()
			{
				static_assert(!std::is_const<ElementType>::value, "CyclicVector does not support const "
					"element types");
				static_assert(std::is_default_constructible<ElementType>::value, "CyclicVector requires "
					"elements to be default constructible");
				static_assert(std::is_copy_constructible<ElementType>::value, "CyclicVector requires "
					"elements to be copy constructible");
				static_assert(std::is_copy_assignable<ElementType>::value, "CyclicVector requires "
			...
			/**
			Constructs a cyclic vector of the given size.
			...
			/**
			Copy constructs a cyclic vector.
			@param vec The cyclic vector to copy */
			...
			/**
			Assignment constructs a cyclic vector.
			@param vec The cyclic vector to copy */
			...
			void next()
			{
				...
				// Advance index
				if (m_iActiveIndex == m_iNumElements - 1)
				{
					m_iActiveIndex = 0;
				}
				else
			...
			/**
			Retrieves the element at the active index. If the container was empty this function will
			...
			/**
			Appends an element to the cyclic vector. If the cyclic vector was full, it will expand to 
			...
			/**
			Pops the last element off of the cyclic vector. If the cyclic vector was empty, no action is taken.
			Note that this function only flags the element as free. Use popAndReset to actually remove the
			element. */
			void pop()
			{
				if (m_iNumElements > 0)
			...
			/**
			Pops the last element off of the cyclic vector. If the cyclic vector was empty, no action is
			taken. */
			void popAndReset()
			{
				if (m_iNumElements > 0)
			...
			/**
			Inserts an element at the given index. If the index is greater than the current number 
			of elements, this function does nothing.
			...
			void insert(const ElementType& element, IndexType iIndex)
			{
				...
				if (iIndex >= m_iNumElements)
				{
					...
					{
						fatalexit("Out of bounds cyclic vector access. Max index: 0"
			...
			/**
			Creates an iterator targetting the first element. When the cyclic vector is empty this
			iterator is equal to the iterator created via a call to end().
			@return An iterator targetting the first element in the cyclic vector
			@see end */
			...
			/**
			Creates a const iterator targetting the first element. When the cyclic vector is empty 
			this iterator is equal to the iterator created via a call to cend().
			@return A const iterator targetting the first element in the cyclic vector */
			...
			/**
			Creates a const iterator targetting the theoretical element one past the last element 
			in the cyclic vector.
